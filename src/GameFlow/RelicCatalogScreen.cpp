...
sf::Texture* get_relic_icon_texture(std::unordered_map<std::string, sf::Texture>& cache, const std::string& id) {
    auto it = cache.find(id);
    if (it != cache.end()) return &it->second;
    const std::string path = resolve_image_path("assets/relics/" + id);
...
void runRelicCatalogScreen(sf::RenderWindow& window) {
    ...
    while (window.isOpen()) {
        ...
        // 顶部标题（黑金风）
        {
            sf::Text t(font, sf::String(L"遗物总览"), 42);
        ...
        for (int i = 0; i < static_cast<int>(ids.size()); ++i) {
            ...
            if (sf::Texture* icon = get_relic_icon_texture(iconCache, ids[static_cast<size_t>(i)])) {
                sf::Sprite sp(*icon);
