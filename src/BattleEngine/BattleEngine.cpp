...
void BattleEngine::start_battle(const std::vector<MonsterId>& monster_ids,  // 开始战斗
    ...
    state_.pendingPlayerBlockVfx.clear();
    state_.pendingMonsterPoisonVfx.clear();
    state_.pendingStrengthVfx.clear();
...
void BattleEngine::tick_damage_displays() {                            // 每帧调用：递减显示时长，移除过期项
    ...
    auto& bl = state_.pendingPlayerBlockVfx;
    bl.erase(std::remove_if(bl.begin(), bl.end(), [](PlayerBlockVfxSignal& ev) { return --ev.frames_remaining <= 0; }),
             bl.end());
    auto& ap = state_.pendingMonsterPoisonVfx;
    ap.erase(std::remove_if(ap.begin(), ap.end(), [](MonsterPoisonVfxSignal& ev) { return --ev.frames_remaining <= 0; }),
             ap.end());
    auto& st = state_.pendingStrengthVfx;
    st.erase(std::remove_if(st.begin(), st.end(), [](StrengthVfxSignal& ev) { return --ev.frames_remaining <= 0; }),
             st.end());
}
...
int burn_end_turn_damage_for_id(const CardId& id) {
 ...
 bool BattleEngine::play_card(int hand_index, int target_monster_index) {  // 打出卡牌
    ...
    if (needs_target) {
        ...
        // 群攻/无目标牌不得携带 UI 残留或越界下标，否则遗物/伤害链会按错误下标访问 monsters[]
    ...
    // 打牌过程中（扣能、疼痛等）可能改变战场；对 on_card_played 使用与当前 state_ 一致的目标下标，避免勒脖等读越界或无效 monsters[]
    int target_for_card_played_events = target_monster_index;
    if (!needs_target) {
        target_for_card_played_events = -1;
    } else {
        const int nm = static_cast<int>(state_.monsters.size());
        if (target_for_card_played_events < 0 || target_for_card_played_events >= nm
            || state_.monsters[static_cast<size_t>(target_for_card_played_events)].currentHp <= 0) {
            target_for_card_played_events = -1;
        }
    }
     modifiers_.on_card_played(state_, played_id, target_for_card_played_events, &card_ctx);  // 广播：打出卡牌（勒脖等）
    ...
    const int block_before_play = state_.player.block;
    for (int i = 0; i < execute_times; ++i) {
        card_system_->execute_effect(played_id, ctx);                    // 执行卡牌效果
    }
    if (cd && cd->cardType == CardType::Skill && state_.player.block > block_before_play)
        state_.pendingPlayerBlockVfx.push_back(PlayerBlockVfxSignal{});
...
void BattleEngine::apply_damage_to_monster(DamagePacket& dmg) {        // 对怪物施加伤害
    ...
    if (damageDisplayAmount > 0) {
        const bool hit_vfx =
            (dmg.source_type == DamagePacket::SourceType::Player && dmg.from_attack);
        state_.pendingDamageDisplays.push_back(DamageDisplayEvent{false, idx, damageDisplayAmount, 180, hit_vfx});
    }
...
void BattleEngine::apply_curse_hand_effects_before_turn_end_discard() {
 ...
 int EffectContext::get_effective_damage_dealt_by_player(int base_damage, int target_monster_index) const {  // 玩家对怪物的有效伤害
     if (!engine_) return base_damage;
     if (target_monster_index != -1) {
         const int n = engine_->get_monster_count_impl();
         if (target_monster_index < 0 || target_monster_index >= n)
             target_monster_index = -1;
     }
     return engine_->get_effective_damage_dealt_by_player_impl(base_damage, target_monster_index, from_attack);
 }
...
size_t BattleEngine::run_rng_uniform_size(size_t lo, size_t hi) {
 ...
 int BattleEngine::get_effective_damage_dealt_by_player_impl(int base_damage, int target_monster_index, bool from_attack) const {  // 玩家对怪物的有效伤害（经 modifier）
     if (target_monster_index != -1 &&
         (target_monster_index < 0 || target_monster_index >= static_cast<int>(state_.monsters.size())))
         target_monster_index = -1;
...
void BattleEngine::apply_status_to_player_impl(StatusId id, int stacks, int duration) {  // 对玩家施加状态（经 modifier 检查）
    const StatusId orig_id = id;
    StatusApplyContext ctx{StatusApplyContext::Target::Player, -1, std::move(id), stacks, duration, false, state_};
    ...
    if (!ctx.blocked) {
        ...
        if (orig_id == "strength" && ctx.stacks > 0)
            state_.pendingStrengthVfx.push_back(StrengthVfxSignal{true, 0, 180});
    }
}
...
void BattleEngine::apply_status_to_monster_impl(int monster_index, StatusId id, int stacks, int duration) {  // 对怪物施加状态
    ...
    const StatusId orig_id = id;
    StatusApplyContext ctx{StatusApplyContext::Target::Monster, monster_index, std::move(id), stacks, duration, false, state_};
    modifiers_.on_before_status_applied(ctx);
    if (!ctx.blocked) {
        add_or_merge_status(state_.monsters[static_cast<size_t>(monster_index)].statuses, std::move(ctx.id), ctx.stacks,
                            ctx.duration);
        if (orig_id == "poison" && ctx.stacks > 0) {
            state_.pendingMonsterPoisonVfx.push_back(MonsterPoisonVfxSignal{monster_index, 180});
            // 与带毒攻击同帧时优先播中毒特效：取消本怪物最近一条待播命中序列帧
            for (auto it = state_.pendingDamageDisplays.rbegin(); it != state_.pendingDamageDisplays.rend(); ++it) {
                if (!it->is_player && it->monster_index == monster_index && it->hit_vfx) {
                    it->hit_vfx = false;
                    break;
                }
            }
        ...
        if (orig_id == "strength" && ctx.stacks > 0)
            state_.pendingStrengthVfx.push_back(StrengthVfxSignal{false, monster_index, 180});
    }
}
