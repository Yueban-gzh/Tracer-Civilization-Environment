...
bool BattleEngine::play_card(int hand_index, int target_monster_index) {  // 打出卡牌
    ...
    const bool use_free_attack = is_attack && get_status_stacks(state_.player.statuses, "free_attack") > 0;  // 下 X 张攻击牌耗能 0

    int cost = cd ? cd->cost : 0;                                      // 基础费用
    if (cost >= 0) {                                                   // 非 X 费：应用本场减费（确立基础等）
        cost -= inst_in_hand.combatCostDiscount;
        if (cost < 0) cost = 0;
    }
    const bool is_x_cost = (cost == -1);
    int x_spent = 0;
    if (use_free_attack) {
        x_spent = 0;                                                   // 免费攻击：不扣能量，X 费攻击也不清空能量
    } else if (is_x_cost) {
    ...
    ctx.x_cost_spent = x_spent;                                        // 记录本次 X 费值（非 X 费为 0）
     ...
     CardPlayContext card_ctx{                                          // 打出卡牌上下文（勒脖、点穴等）
         ...
         [this](int n) { if (card_system_ && n > 0) card_system_->draw_cards(n); },  // 散热等：抽牌
