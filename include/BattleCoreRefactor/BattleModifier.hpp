...
struct TurnStartContext {                       // 玩家回合开始时的上下文
    ...
    std::function<void(CardId)> generate_to_hand;  // 生成牌入手牌
    /** 立即杀死玩家并触发 on_player_died（渎神等；不走扣血链，缓冲无法抵消） */
    std::function<void()> instant_kill_player;
    /** 将 n 张随机「普通稀有度」牌以临时牌形式置入手牌（你好等；满手则按 CardSystem 规则入弃牌） */
    std::function<void(int n)> add_random_common_cards_to_hand;
    /** 增加玩家格挡并触发势不可挡等（替代直接改 state.player.block） */
    std::function<void(int amount)> grant_player_block;
};
...
struct PlayerTurnEndContext {                   // 玩家回合结束时的上下文
    ...
    /** 对玩家造成伤害（走格挡、缓冲、无实体等；缠绕等） */
    std::function<void(int amount)> deal_damage_to_player;
    /** 对当前手牌中每张带 Retain 的牌叠加 combatCostDiscount（由确立基础等传入 delta） */
    std::function<void(int discount_delta)> apply_combat_discount_to_retained_hand_cards;
    /** 增加玩家格挡并触发势不可挡等（金属化、奥利哈钢等） */
    std::function<void(int amount)> grant_player_block;
};
...
struct EnemyTurnContext {                       // 敌方回合时的上下文
    ...
    std::function<void(int monster_index)> kill_monster;  // 消逝等：强制死亡并走死亡事件（非伤害）
};
...
struct CardPlayContext {                        // 打出卡牌时的上下文
    ...
    std::function<void(int n)> draw_cards;      // 抽 n 张牌（散热等用）
    bool is_power = false;                      // 是否为能力牌
    std::function<void(int amount)> deal_damage_to_player;  // 死亡律动等：对玩家结算伤害（走格挡、缓冲等）
    /** 增加玩家格挡并触发势不可挡等（残像等） */
    std::function<void(int amount)> grant_player_block;
};

/** 玩家实际增加格挡之后广播（势不可挡：对随机存活敌人造成伤害） */
struct PlayerBlockGainedContext {
    std::function<void(int damage_amount)> deal_damage_to_random_living_monster;  // 对随机存活敌人造成指定伤害（尊重格挡）
};

/** 伤害结算后的上下文：百年积木等可抽牌；其它监听玩家受伤的效果可扩展 */
struct DamageAppliedContext {
    std::function<void(int monster_index, int amount)> deal_damage_to_monster_ignoring_block;  // 对怪物造成无视格挡伤害（预留）
...
public:
    ...
    virtual void on_damage_applied(const DamagePacket& dmg, BattleState& state, DamageAppliedContext* ctx) { (void)ctx; }  // 伤害结算后
    virtual void on_turn_start_player(BattleState& state, TurnStartContext* ctx) { (void)ctx; }  // 玩家回合开始：中毒扣血、抽牌数修改等
    /** 格挡结算之后、扣除 currentHp 之前；可改写 hpDamage（如缓冲令本次实际掉血为 0）。任意来源伤害均会经过。 */
    virtual void on_player_before_hp_loss(int& hpDamage, BattleState& state, const DamagePacket& dmg) {
        (void)hpDamage;
        (void)state;
        (void)dmg;
    }
    /** 格挡结算之后、扣除怪物 currentHp 之前；可改写 hpDamage（如坚不可摧本回合受伤上限）。 */
    virtual void on_monster_before_hp_loss(int& hpDamage, BattleState& state, int monster_index, const DamagePacket& dmg) {
        (void)hpDamage;
        (void)state;
        (void)monster_index;
        (void)dmg;
    }
    /** 玩家回合结束、手牌移入弃牌堆之前；将 skip_full_discard_ref 置 true 则跳过常规弃牌（均衡）；虚无牌仍由引擎单独送入消耗堆。 */
    virtual void on_before_player_hand_discard(BattleState& state, bool& skip_full_discard_ref) {
        (void)state;
        (void)skip_full_discard_ref;
    }
    ...
    /** 玩家格挡数值已增加 amount 后调用（势不可挡等） */
    virtual void on_player_block_gained(BattleState& state, int amount, PlayerBlockGainedContext* ctx) {
        (void)state;
        (void)amount;
        (void)ctx;
    }
 ...
 * - 怪物状态(200-299)：毒/勒脖等
 * - 系统级(300+)：格挡清零等；玩家 duration 递减在玩家回合结束、怪物在敌方回合结束（见 DurationTickModifier）
 */
/** 缓冲：先于遗物与其它状态，在扣血前判定是否免本次生命值损失 */
constexpr int MOD_PRIORITY_BUFFER_PRE = -100;
...
public:
    ...
    void on_damage_applied(const DamagePacket& dmg, BattleState& state, DamageAppliedContext* ctx) {
        for (auto& p : modifiers_) p.second->on_damage_applied(dmg, state, ctx);  // 广播：伤害结算后
    }
    ...
    void on_player_before_hp_loss(int& hpDamage, BattleState& state, const DamagePacket& dmg) {
        for (auto& p : modifiers_) p.second->on_player_before_hp_loss(hpDamage, state, dmg);
    }
    void on_monster_before_hp_loss(int& hpDamage, BattleState& state, int monster_index, const DamagePacket& dmg) {
        for (auto& p : modifiers_) p.second->on_monster_before_hp_loss(hpDamage, state, monster_index, dmg);
    }
    void on_before_player_hand_discard(BattleState& state, bool& skip_full_discard_ref) {
        for (auto& p : modifiers_) {
            bool local_skip = false;
            p.second->on_before_player_hand_discard(state, local_skip);
            if (local_skip) skip_full_discard_ref = true;
        }
    }
    ...
    void on_player_block_gained(BattleState& state, int amount, PlayerBlockGainedContext* ctx) {
        for (auto& p : modifiers_) p.second->on_player_block_gained(state, amount, ctx);
    }
