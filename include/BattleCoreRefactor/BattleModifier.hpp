...
public:
    ...
    virtual void on_turn_end_player(BattleState& state) {}                                    // 玩家回合结束：金属化加格挡等
...
public:
    ...
    void on_turn_end_player(BattleState& state) {
        for (auto& p : modifiers_) p.second->on_turn_end_player(state);  // 广播：玩家回合结束
    }
